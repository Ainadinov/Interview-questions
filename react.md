...
# 5. Что такое подём состояния?

`Подём состояния` -  внутренное состояние одного компонента поднимается выше по ирархи (State lifting)

# 6. Что такое React фрагмент?

`React фрагмент` - используеться для оборочивание компонента для одного безполезного диво кторое не используется

# 7. Жизненный цикл компонентов?
    
`Component Lifecycle` - каждый компонент React проходит несколько стадий в процессе своей жизни: он создается, затем добавляется в DOM, получает пропсы, и, наконец, удаляется из дерева. 
    Стадий Lifecycle:
        1. Монтирование появление первое что вызывается constructor после этого вызывается render, рендерется вёрстка или интерфейс
            - обновляется DOM структуры рефы после этого вызывается ComponentDidMount()
        2. Обновление - props, state появляется после этого render новыми данными которые приходит от props, state и изменяет DOM структуру
            - после изменение данных вызывается ComponentDidUpdate()
            - каждый раз когда компонент обновөя выполяется этот цепочка действий
        3. Когда компонент полнөю исчезает из страницы вызывается ComponentWillUnmount()

# 8. Что такое ComponentDidMount()?

`ComponentDidMount()` - вызывается после рендеринга компонента. Здесь можно выполнять запросы к удаленным ресурсам
                    ** Этот метод — подходящее место для настройки подписки. 
                    ** Если активировать этот функциюб не забудьте отказаться от подписки в componentWillUnmount().
...
const MovieDB = 'https://api.themoviedb.org/3/';
    ...
    class App extends Component{
        ...
        componentDidMount(){
            this.fetchMovies()
            this.timerId = setInterval(this.fetchMovies, 3000)
        }
