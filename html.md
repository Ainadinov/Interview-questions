...
# 19. Что такое категории контента в HTML5?

`Контентная модель` - описывает какой тип следует ожидат внутри элемента и какие элементы могут быть вложены

                    ** Мета содержимое - содержить инфо об HTML документе (link, meta script, title)
                    ** Потоковое содержимое - элементы которое используется в теле документа (a, input, img, var)
                    ** Секционные содержимое - область зоголовок и колонтитулов (section, nav, article)
                    ** Зоголовочно содержимое - наличие зоголовок (h1, h2, h3, h4, h5, h6)
                    ** Текстовое содержимое - элементы для разметки текста внутри абзацев
                    ** Встаиваемое содержимое - содержимое которое импор-я в документ из других источников

# 20. Что такое SVG и Canvas?

`SVG` - используется для размешение векторное изображение на веб-страницы

        ** состоит из фигур
        ** можно изменит через CSS и JS
        ** становиться частью DOM дерева
        ** можеть быть напечатоно с высоким качеством

`Canvas` - програмисты даёт браузеру команды, как ему нужно что-то нарисовать (для содание игр)

        ** состоит из пикселей
        ** кажыдый элемент похож на IMG
        ** модифи-я только через JS
        ** Плохо масшт-я

# 21. Для чего нужен атрибут Autocomplete?

`Autocomplete` - атрибут при помощи которого форма запольняется один клик

                ** off - отключает автозаполнение
                ** on - значение по-умол, разреш автозаполнение

# 22. Что такое output в HTML5?

`Output` - позволяет выводить результаты вычислений или действий польз-я. Исполь=я тех случаях, когда поль-лю надо показать результат работы в реальным времени

# 23. Что такое свойтсво valueAsNumber?

`valueAsNumber` - свойство для полей формыб возвращаеть значение в виде числа, а не строк.

# 24. Типы узлов DOM-дерева?

        Узнать тип узла в DOM-дерева можно через NodeType
                ** элемент
                ** атрибут
                ** текстовый узел
                ** комментарий
